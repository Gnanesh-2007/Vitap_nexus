...
def _parse_events(cell) -> list[CalendarEventModel]:
    ...
    or with `<font>`, `<b>`, `<strong>`, `<p>`, `<div>` or `<br>` separated text.
    The parenthesised entries qualify the ones before them, so lines are folded into
    events that way — which also tolerates a day carrying more than one entry (e.g. LAB FAT).
    """
    # Extract child elements while preserving order
    lines: list[str] = []
    
    elements = cell.find_all(["span", "font", "b", "strong", "p", "div", "a"])
    if elements:
        for el in elements:
            txt = _clean(el)
            # Exclude lone day numbers and duplicates
            if txt and not txt.isdigit() and txt not in lines:
                lines.append(txt)

    # Fallback to line breaks in cell text if elements missed content
    for br in cell.find_all(["br"]):
        br.replace_with("\n")
    cell_lines = [l.strip() for l in cell.get_text(separator="\n").splitlines() if l.strip()]
    for l in cell_lines:
        if not l.isdigit() and l not in lines:
            lines.append(l)
    ...
    try:
        ...
        for row in rows[1:]:
            for index, cell in enumerate(row.find_all("td")):
                ...
                number = None
                if spans:
                    first_text = _clean(spans[0])
                    if first_text.isdigit():
                        number = first_text

                if number is None:
                    # Try first token from cell text
                    tokens = _clean(cell).split()
