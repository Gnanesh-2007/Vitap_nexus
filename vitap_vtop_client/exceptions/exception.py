...
class VtopAttendanceError(VitapVtopClientError):
    """Raised when fetching attendance fails due to data parsing, invalid semSubId, server-side validation, etc."""
...
class VtopGradeHistoryError(VitapVtopClientError):
    """Raised when fetching grade history fails due to data parsing, invalid semester id, server-side validation, etc."""
...
class VtopMentorError(VitapVtopClientError):
    """Raised when fetching faculty mentor details fails due to data parsing, server-side validation, etc."""
...
class VtopBiometricError(VitapVtopClientError):
    """Raised when fetching biometric logs fails due to data parsing, invalid date, server-side validation, etc."""
...
class VtopProfileError(VitapVtopClientError):
    """Raised when fetching student profile fails due to data parsing, server-side validation, etc."""
...
class VtopSemesterError(VitapVtopClientError):
    """Raised when fetching available semesters fails due to data parsing, server-side validation, etc."""

    def __init__(self, message: str, status_code: int | None = None):
        super().__init__(message, status_code)


class VtopFacultyError(VitapVtopClientError):
    """Raised when searching faculty or fetching faculty details fails due to parsing, invalid parameters, etc."""

    def __init__(self, message: str, status_code: int | None = None):
        super().__init__(message, status_code)


class VtopPaymentsError(VitapVtopClientError):
    """Raised when fetching pending payments or payment receipts fails due to parsing or server-side validation."""

    def __init__(self, message: str, status_code: int | None = None):
        super().__init__(message, status_code)


class VtopCoursePageError(VitapVtopClientError):
    """Raised when fetching course page information, course materials, or slots fails."""

    def __init__(self, message: str, status_code: int | None = None):
        super().__init__(message, status_code)


class VtopOutingError(VitapVtopClientError):
    """Base exception for hostel outing operations (general, weekend, leave passes, cancellations)."""

    def __init__(self, message: str, status_code: int | None = None):
        super().__init__(message, status_code)


class VtopGeneralOutingError(VtopOutingError):
    """Raised when fetching, applying for, or deleting general outing requests fails."""

    def __init__(self, message: str, status_code: int | None = None):
        super().__init__(message, status_code)


class VtopWeekendOutingError(VtopOutingError):
    """Raised when fetching, applying for, or deleting weekend outing requests fails."""
